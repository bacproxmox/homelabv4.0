...
function App(): JSX.Element {
  ...
  const [githubRepoName, setGithubRepoName] = useState('homelabv4.0');
  ...
  async function refreshAgent(): Promise<void> {
    const health = await window.homelab.agentRequest<AgentHealth>('/api/v1/health');
    setAgentHealth(health);
    const manifest = await window.homelab.agentRequest<{ steps: GuidedStep[] }>('/api/v1/manifest');
    setSteps(manifest.steps);
    const catalog = await window.homelab.agentRequest<ScriptCatalog>('/api/v1/script-catalog');
    setScriptCatalog(catalog);
    const state = await window.homelab.agentRequest<{
      steps: StepState[];
      scripts?: ScriptState[];
      runs: RunInfo[];
    ...
    try {
      const packs = await window.homelab.agentRequest<{ packs: BrandingPack[] }>('/api/v1/branding/packs');
  ...
  async function startRun(target: string): Promise<RunInfo> {
    const run = await window.homelab.agentRequest<RunInfo>('/api/v1/runs', {
      method: 'POST',
      body: { target }
    });
  ...
  async function startScript(item: ScriptCatalogItem): Promise<void> {
    ...
    const run = await window.homelab.agentRequest<RunInfo>('/api/v1/script-catalog/runs', {
      method: 'POST',
      body: { id: item.id }
    });
  ...
  async function startScriptGroup(group: ScriptCatalogGroup): Promise<void> {
    ...
    const run = await window.homelab.agentRequest<RunInfo>(`/api/v1/script-catalog/groups/${group.id}/run`, {
      method: 'POST'
    });
    setRuns((current) => [run, ...current.filter((entry) => entry.id !== run.id)]);
    setSelectedRunId(run.id);
  ...
  async function waitForRunCompletion(runId: string, timeoutMs = 5 * 60 * 1000): Promise<RunInfo> {
    ...
    while (Date.now() < deadline) {
      await delay(2500);
      const state = await window.homelab.agentRequest<{ runs: RunInfo[] }>('/api/v1/state');
      setRuns(state.runs);
  ...
  async function createSupportBundle(): Promise<void> {
    ...
    const finalLog = await window.homelab
      .agentRequest<{ text: string }>(`/api/v1/runs/${run.id}/logs`)
  ...
  async function refreshRunLog(runId: string, options?: { quiet?: boolean }): Promise<void> {
    try {
      const log = await window.homelab.agentRequest<{ text: string }>(`/api/v1/runs/${runId}/logs`, { timeoutMs: 10_000 });
  ...
  async function loadRemoteTrueNasIso(): Promise<TrueNasIsoSelection | null> {
    try {
      const state = await window.homelab.agentRequest<{ installProfile?: { truenasIso?: TrueNasIsoSelection } }>('/api/v1/state');
                ...
                <button
                  onClick={() =>
                    void runAction('Collect hardware inventory', async () => {
                      const inventory = await window.homelab.agentRequest<HardwareInventory>('/api/v1/inventory/hardware');
          ...
          <section className="panel-grid">
            ...
            <div className="section-panel quiet">
              ...
              <ol className="process-list">
                ...
                <li>Select an existing version repo or type a new repo name like `homelabv4.0`.</li>
                <li>Select `homelabv4.0.zip`.</li>
    ...
    <div className="section-panel full-install-panel">
      ...
      {failedStep && (
        <div className="warning-block compact-warning failed-warning">
          ...
          <p>The latest run log is shown in the Runs & Logs panel.</p>
        </div>
      )}
    </div>
  );
      ...
      <div className="github-layout">
        ...
        <div className="github-publish-card">
          ...
          <div className="form-grid">
            <label>
              Repository Name
              <input value={repoName} onChange={(event) => onRepoNameChange(event.target.value)} placeholder="homelabv4.0" />
            </label>
            <label>
              Branch
