...
find_jmicron_sata(){ lspci -Dnn | awk 'BEGIN{IGNORECASE=1} /SATA controller|AHCI/ && /JMicron|JMB|JMS|JMB58|JMS58|JMB585|JMB582/ {print $1}' | sort -u; }

stop_if_running(){ local vm="$1"; if qm status "$vm" 2>/dev/null | grep -q running; then qm shutdown "$vm" --timeout 60 || qm stop "$vm" || true; fi; }
start_and_wait(){ local vm="$1"; qm start "$vm" || true; wait_for_agent "$vm" 80 || true; wait_ssh "$vm" || true; }
...
attach_vm107_hardware(){
  ...
  [[ "$count" -gt 0 ]] && echo "✅ VM107 JMicron controller count: $count" || echo "⚠️ JMicron/JMB/JMS SATA controller bulunamadı."
}

fix_vm107_driver_and_disks(){
  echo "🔧 VM107 NVIDIA/JMicron/Chia disk kontrolü..."
  wait_ssh 107
...
echo "🧪 GPU + Chia hardware passthrough repair başlıyor..."
attach_vm106 || true
...
echo "✅ GPU/Chia passthrough repair tamamlandı."
